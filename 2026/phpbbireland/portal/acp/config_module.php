...
*
* @copyright (c) 2014 Michael O’Toole <http://www.michaelo2l.com>
...
class config_module
{
	...
	function main($module_id, $mode)
	{
		...
		if (isset($data) && $config['portal_version'])
		{
		...
		$template->assign_vars(array(
			...
			'MOD_LATEST_VERSION'   => isset($data['version']) ? $data['version'] : $user->lang('NO_SOURCE_FOUND'),
			...
			'S_OPT'             => 'configure',
			'S_MOD_DATA'        => (isset($data['version'])) ? $data['version'] : $portal_build,
		));
		...
		switch ($mode)
		{
			case 'save':
			{
				...
				$config->set('portal_build',	$request->variable('portal_build', '3.3.0'));
